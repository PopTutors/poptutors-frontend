...
import { DashboardAssignment, DashboardFileIcon, DashboardLiveHelp, DashboardSession } from '../../../assets/managers'; // must be a string path
...
const StatCard: React.FC<StatCardProps> = ({ title, value, color, svg }) => (
  <div
  ...
  >
    ...
    <img src={svg} alt="icon" className="absolute w-[20px] h-[20px] top-4 right-4" />
  </div>
);
  ...
  return (
    <div className="grid gap-4 mb-6 xsm:grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
      <div className='cursor-pointer' onClick={(e) => setStat("assignment")}>
        <StatCard title="Assignments" value={assignmentsCount} color="bg-[#BCEFFF]" svg={DashboardAssignment} />
      </div>
      <div className='cursor-pointer' onClick={(e) => setStat("liveHelp")}>
        <StatCard title="Exam Help" value={sessionsCount} color="bg-[#FFCED4]" svg={DashboardLiveHelp} />
      </div>
      <div className='cursor-pointer' onClick={(e) => setStat("sessions")}>
        <StatCard title="Sessions" value={liveHelpsCount} color="bg-[#FFE4BB]" svg={DashboardSession} />
      </div>
    </div>
  );
};
