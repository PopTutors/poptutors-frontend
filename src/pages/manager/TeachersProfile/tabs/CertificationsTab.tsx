...
const CertificationsTab: React.FC<CertificationsTabProps> = ({ profile }) => {
      ...
      {certifications.length > 0 ? (
        <div className="space-y-6">
          {certifications.map((cert, index) => (
            <div
              key={cert.id || index}
              className={`flex gap-4 pb-4 ${index !== certifications.length - 1 ? "border-b mb-2" : ""
                }`}
            >
              {/* Avatar (Google-like icon for now) */}
              ...
              <div className="flex-1">
                <h3 className="text-[22px] font-semibold text-[#141414] mb-1">
                  {cert.title}
                </h3>
                {cert.issuer && (
                ...
                <div className="flex flex-wrap gap-4 text-[16px] text-[#595959] mb-3">
